...
### フェーズ3: 📁階層設計
@instructions/directory_patterns内のファイルを参考にして、ユーザの要件に合致する📁の階層設計を行うこと。

/Agent-Shared/directory_map.txtなどに📁階層を示すこと。PMが適宜参照し、更新等を行うこともある。ただし、末端はworkerが存在する📁まで記載する。workerがそれ以降のディレクトリに自由に作成する📁は含めなくて良い。
